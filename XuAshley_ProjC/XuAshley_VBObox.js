...
function VBObox0() {
  ...
  this.vboVerts = (2 * (xcount + ycount)); // # of vertices held in 'vboContents' array
  this.vboContents = new Float32Array(floatsPerVertex * (2 * (xcount + ycount)));
...
function VBObox1() {
  ...
  this.FRAG_SRC = 
    ...
    '  vec3 lightDirection = normalize(u_LampSet[0].pos.xyz - v_Position.xyz);\n' +
...
VBObox1.prototype.initVertexBuffers = function (gl) { // Create a sphere
  ...
  this.lilypadStart = this.vboContents.length / 7;
  this.lilypadLength = lilypadVerts.length / 7;
  for (i = 0; i < lilypadVerts.length; i++) {
    this.vboContents.push(lilypadVerts[i]);
  }

  this.frogStart = this.vboContents.length / 7;
  this.frogLength = frogVerts.length / 7;
  for (i = 0; i < frogVerts.length; i++) {
    this.vboContents.push(frogVerts[i]);
  }
...
VBObox1.prototype.draw = function () {
  ...
  pushMatrix(this.modelMatrix);
  {
    this.modelMatrix.translate(-2.0, 3.0, 1.5);
    this.modelMatrix.rotate(90.0, 1, 0, 0);
    this.modelMatrix.rotate(g_frogAngle1, 0, 1, 0);

    this.drawLilyPad(g_matl2);
    pushMatrix(this.modelMatrix);
    {
      this.modelMatrix.scale(0.6, 0.6, 0.6);
      this.modelMatrix.translate(0.0, 0.5, 0.0);
      this.modelMatrix.rotate(g_frogAngle2, 0, 1, 0);
      this.drawFrog(g_matl1);
    }
    this.modelMatrix = popMatrix();
  ...
  pushMatrix(this.modelMatrix);
  {
    this.modelMatrix.translate(4.0, -2.0, 1.0);
    this.modelMatrix.rotate(90, 0, 1, 0);
    this.modelMatrix.rotate(g_angleNow, 0, 0, 1);
    this.drawFrog(g_matl2);
    pushMatrix(this.modelMatrix);
    {
      this.modelMatrix.scale(0.6, 0.6, 0.6);
      this.modelMatrix.translate(0.0, 1.6, 0.0);
      this.modelMatrix.rotate(30, 0, 0, 1);
      this.drawFrog(g_matl3);

      pushMatrix(this.modelMatrix);
      {
        this.modelMatrix.scale(0.6, 0.6, 0.6);
        this.modelMatrix.translate(0.4, 1.6, 0.0);
        this.modelMatrix.rotate(30, 0, 0, 1);
        this.drawFrog(g_matl1);
      }
    }
  ...
  pushMatrix(this.modelMatrix);
  {
    this.modelMatrix.translate(-2.0, -2.0, 2.0);
    this.modelMatrix.scale(0.5, 0.5, 0.5);
    this.modelMatrix.rotate(g_frogAngle3, 0, 1, 0);
    this.drawSphere(g_matl1);

    pushMatrix(this.modelMatrix);
    {
      //this.modelMatrix.scale(0.5, 0.5, 0.5);
      this.modelMatrix.rotate(90, 0, 1, 0);
      this.modelMatrix.translate(0, 0.0, 1.5);
      this.drawFrog(g_matl3);
    }
  }
  this.modelMatrix = popMatrix();
...
VBObox1.prototype.drawLilyPad = function (matl) {
  // Update the mvp matrix
  this.mvpMatrix.set(mvpMatrix);
  this.mvpMatrix.multiply(this.modelMatrix);
  ...
  gl.drawArrays(gl.TRIANGLE_STRIP, this.lilypadStart, this.lilypadLength);
}

VBObox1.prototype.drawFrog = function (matl) {
  // Update the mvp matrix
  this.mvpMatrix.set(mvpMatrix);
  this.mvpMatrix.multiply(this.modelMatrix);
  ...
  gl.drawArrays(gl.TRIANGLE_STRIP, this.frogStart, this.frogLength);
}

VBObox1.prototype.drawBeeBody = function (matl) {
  // Update the mvp matrix
  this.mvpMatrix.set(mvpMatrix);
  this.mvpMatrix.multiply(this.modelMatrix);
  ...
  gl.drawArrays(gl.TRIANGLE_STRIP, this.beebodyStart, this.beebodyLength);
}

VBObox1.prototype.drawBeeWing = function (matl) {
  // Update the mvp matrix
  this.mvpMatrix.set(mvpMatrix);
  this.mvpMatrix.multiply(this.modelMatrix);
  ...
  gl.drawArrays(gl.TRIANGLE_STRIP, this.beewingStart, this.beewingLength);
}
...
function VBObox2() {
  ...
  this.VERT_SRC =	//--------------------- VERTEX SHADER source code 
    ...
    '  vec3 diffuse = u_Lamp0Diff * u_Kd * nDotL;\n' +
    '  float s = pow(nDotH, u_Kshiny);\n' +
    '  vec3 speculr = u_Lamp0Spec * u_Ks * s;\n' +
    '  v_Color = vec4(diffuse + ambient + emissive + speculr, 1.0);\n' + 
...
VBObox2.prototype.init = function () {
  ...
  if (!this.u_Ke || !this.u_Ka || !this.u_Kd || !this.u_Ks || !this.u_Kshiny) { // || !this.u_Kshiny) {
    console.log('Failed to get the Phong Reflectance storage locations');
    return;
  }
...
VBObox2.prototype.drawSphere = function (matl) {
  ...
  gl.uniform3f(this.u_Kd, Kd[0], Kd[1], Kd[2]);       // Kd	diffuse
  gl.uniform3f(this.u_Ks, Ks[0], Ks[1], Ks[2]);       // Ks specular
  gl.uniform1f(this.u_Kshiny, Kshiny);               // Kshiny shinyness exponent

  gl.drawElements(gl.TRIANGLES, this.vboVerts, gl.UNSIGNED_SHORT, 0);
}

VBObox2.prototype.drawLilyPad = function (matl) {
  // Update the mvp matrix
  this.mvpMatrix.set(mvpMatrix);
  this.mvpMatrix.multiply(this.modelMatrix);
...
VBObox2.prototype.drawFrog = function (matl) {
  // Update the mvp matrix
  this.mvpMatrix.set(mvpMatrix);
  this.mvpMatrix.multiply(this.modelMatrix);
...
VBObox3.prototype.drawLilyPad = function (matl) {
  // Update the mvp matrix
  this.mvpMatrix.set(mvpMatrix);
  this.mvpMatrix.multiply(this.modelMatrix);
...
VBObox3.prototype.drawFrog = function (matl) {
  // Update the mvp matrix
  this.mvpMatrix.set(mvpMatrix);
  this.mvpMatrix.multiply(this.modelMatrix);
...
VBObox4.prototype.drawLilyPad = function (matl) {
  // Update the mvp matrix
  this.mvpMatrix.set(mvpMatrix);
  this.mvpMatrix.multiply(this.modelMatrix);
...
VBObox4.prototype.drawFrog = function (matl) {
  // Update the mvp matrix
  this.mvpMatrix.set(mvpMatrix);
  this.mvpMatrix.multiply(this.modelMatrix);
